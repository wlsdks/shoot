...
class TokenControllerTest {
    ...
    fun `리프레시 토큰으로 새 액세스 토큰을 발급한다`() {
        ...
        `when`(refreshTokenUseCase.generateNewAccessToken(refreshTokenHeader)).thenReturn(loginResponse)
        ...
        verify(refreshTokenUseCase).generateNewAccessToken(refreshTokenHeader)
    }
}
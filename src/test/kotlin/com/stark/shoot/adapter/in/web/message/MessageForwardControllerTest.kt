...
class MessageForwardControllerTest {
    ...
    fun `메시지를 다른 채팅방으로 전달하고 전달된 메시지를 반환한다`() {
        ...
        verify(forwardMessageUseCase).forwardMessage(
            MessageId.from(originalMessageId),
            ChatRoomId.from(targetRoomId),
            UserId.from(forwardingUserId)
        )
    ...
    fun `메시지를 특정 사용자에게 전달하고 전달된 메시지를 반환한다`() {
        ...
        `when`(forwardMessageToUserUseCase.forwardMessageToUser(
            MessageId.from(originalMessageId),
        ...
        verify(forwardMessageToUserUseCase).forwardMessageToUser(
            MessageId.from(originalMessageId),
            UserId.from(targetUserId),
            UserId.from(forwardingUserId)
        )

...
class TypingIndicatorMessagesService(
    ...
    @Value("\${app.typing-indicator.cleanup-interval-minutes:10}") private val cleanupIntervalMinutes: Long = 10
    ...
    override fun sendMessage(message: TypingIndicatorMessage) {
        try {
            ...
            val lastSentStr = try {
                redisTemplate.opsForValue().get(redisKey) ?: "0"
            } catch (e: Exception) {
                logger.warn(e) { "Redis에서 타이핑 상태 조회 실패, 로컬 캐시 사용: $key" }
                localTypingCache.getOrDefault(key, 0L).toString()
            }
            val lastSent = lastSentStr.toLongOrNull() ?: 0L

            // 이전 타이핑 상태 확인
            val previousTypingState = try {
                val stateKey = "${redisKey}:state"
                val state = redisTemplate.opsForValue().get(stateKey)
                state?.toBoolean() ?: false
            } catch (e: Exception) {
                false
            }
            ...
            if (message.isTyping != previousTypingState || now - lastSent > RATE_LIMIT_MS) {
                ...
                try {
                    redisTemplate.opsForValue().set(redisKey, now.toString(), Duration.ofMillis(ENTRY_EXPIRY_MS))
                    redisTemplate.opsForValue()
                        .set("${redisKey}:state", message.isTyping.toString(), Duration.ofMillis(ENTRY_EXPIRY_MS))
                } catch (e: Exception) {
                    logger.warn(e) { "Redis에 타이핑 상태 저장 실패, 로컬 캐시 사용: $key" }
        ...
        try {
            ...
            for (redisKey in keys) {
                if (!redisKey.endsWith(":state")) { // 상태 키는 건너뜀
                    try {
                        val lastTypingStr = redisTemplate.opsForValue().get(redisKey) ?: continue
                        val lastTyping = lastTypingStr.toLongOrNull() ?: continue

                        // 타이핑 상태 확인
                        val stateKey = "$redisKey:state"
                        val isTypingStr = redisTemplate.opsForValue().get(stateKey) ?: continue
                        ...
                        if (isTyping && now - lastTyping > STOPPED_TYPING_DELAY_MS) {
                            ...
                            if (keyParts.size >= 2) {
                                val userId = keyParts[0].toLongOrNull() ?: continue
                                val roomId = keyParts[1].toLongOrNull() ?: continue
                                ...
                                logger.debug { "타이핑 중지 감지: 사용자=$userId, 채팅방=$roomId (${now - lastTyping}ms 동안 타이핑 없음)" }
                                ...
                                // 상태 업데이트
                                redisTemplate.opsForValue().set(stateKey, "false", Duration.ofMillis(ENTRY_EXPIRY_MS))
                            }
                        }
                    } catch (e: Exception) {
